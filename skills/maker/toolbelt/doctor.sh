...
cat > "$MAKER_HOME/capabilities.json" <<JSON
{
  ...
  "keys":   { "freesound": "$([ -n "${FREESOUND_API_KEY:-}" ] && echo yes || echo no)",
              "elevenlabs": "$([ -n "${ELEVENLABS_API_KEY:-}" ] && echo yes || echo no)",
              "tenor": "$([ -n "${TENOR_API_KEY:-}" ] && echo yes || echo no)",
              "pexels": "$([ -n "${PEXELS_API_KEY:-}" ] && echo yes || echo no)" },
