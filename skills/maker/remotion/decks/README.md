...
| File | For |
|---|---|
| `showcase.json` | **every new shape once** (its `shots/` and `logos/` paths are placeholders: `mk logo get` the brands, drop your captures in `public/shots/`) — kinetic, chapter, split, versus, steps, timeline, checklist, charts, orbit, gallery, focus, before/after, notifications, post, layers, transitions |
| `example.json` | the reference studio short |
